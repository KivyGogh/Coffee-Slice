...
var exportBaseSmallSvg = function(context) {
                    ...
                    for (var j=0; j < [selection count]; j++) {
                        ...
                        new AppSandbox().authorize(path, function() {
                            for (var f=0; f < factors.length; f++) {
                                ...
                                slice1x = [MSExportRequest requestWithRect:[[sliceSmall absoluteRect] rect] scale:1];
                                [doc saveArtboardOrSlice:slice1x toFile:path + "/" + [layer name] + "_@1x" + ".svg"];
                                slice2x = [MSExportRequest requestWithRect:[[sliceMiddle absoluteRect] rect] scale:1];
                                [doc saveArtboardOrSlice:slice2x toFile:path + "/" + [layer name] + "_@2x" + ".svg"];
                                slice3x = [MSExportRequest requestWithRect:[[sliceBig absoluteRect] rect] scale:1];
                                [doc saveArtboardOrSlice:slice3x toFile:path + "/" + [layer name] + "_@3x" + ".svg"];
                            }
                        });
